...
const report = (list) => {
  ...
  return `有效人数${valid.length}人，平均${average(valid)}分，最高${highest(valid).score}分（${highest(valid).name}）；等级分布：A${dist.A}人 B${dist.B}人 C${dist.C}人 D${dist.D}人 F${dist.F}人；不及格名单：${failed(valid).join('、') || '无'}`;
};
